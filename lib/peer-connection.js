...
class PeerConnection {
  ...
  kill (err = null, eosDetected = false) {
    ...
    if (this.state === STATE_DEAD) {
      if (err) console.warn('Warning kill() invoked on dead peer-connection\n', this.shortid, err)
      return
    }
  ...
  sendManifest (namespace, manifest, cb) {
    ...
    this.debug(`manifest#${mid} sent with ${manifest.length} keys`)
  }
  ...
  _onChannelClosed (dk, pk) {
    ...
    if (pk) { // For established channels where the public-key is known.
