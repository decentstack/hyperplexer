  ...
  constructor (initiator, exchangeKey, opts = {}) {
    ...
    this.exchangeExt = CoreExchangeExtension(this, {
      onmanifest: (snapshot, accept, peer) => {
        ...
        // Forward upwards
        if (typeof this.handlers.onmanifest === 'function') this.handlers.onmanifest(snapshot, accept, this)
      },
      onrequest: (req, peer) => {
        peer.debug(`Received replication request for ${req.keys.length} feeds`)
        // TODO: don't forward the peer, forward this connection.
        // PeerConnection is the peer
        // Forward upward
